...
Comply with the authoritative write policy in `Allowed Writes and State Boundaries` throughout the audit. Do not create fixes, commits, pull requests, migrations, snapshots, dependency updates, formatting changes, or rewritten tests.

## Allowed Writes and State Boundaries

The future audit may intentionally create or change only:

- The required canonical final HTML report, including replacement of that exact report only after the output-collision preflight obtains explicit user authorization.
- Generated restore, build, test, coverage, analyzer, and package-diagnostic artifacts in ignored or temporary locations under the repository root.
- Necessary OS-temporary files and external package or tool-cache state caused by approved diagnostics, such as the global NuGet cache.

The canonical report is the sole intended non-ignored repository output. No other tracked or non-ignored untracked repository file may be intentionally changed. Existing application source, configuration, dependencies, migrations, tests, and documentation remain untouched apart from an explicitly authorized replacement of the canonical report. Approved diagnostics must not write to uncontrolled persistent databases, storage, email, browsers, production-like services, or other external systems.
...
10. Confirm compliance with `Allowed Writes and State Boundaries` and identify whether the canonical report was created or explicitly authorized for replacement.
...
5. Confirm compliance with `Allowed Writes and State Boundaries` and identify whether the report was newly created or replaced with explicit authorization under the collision preflight.
